...
function(es_make_c_cxx_runtime_flags)
    ...
    if("${CMAKE_C_COMPILER_ID}" STREQUAL "MSVC" OR "${CMAKE_CXX_COMPILER_ID}" STREQUAL "MSVC")
        if(ARG_STATIC_RUNTIME)
            set(debug_flags "${msvc_option_prefix}MTd")
            set(release_flags "${msvc_option_prefix}MT")

            if(ARG_RESULT_MSVC_RUNTIME_LIBRARY)
                set(${ARG_RESULT_MSVC_RUNTIME_LIBRARY} "MultiThreaded$<$<CONFIG:Debug>:Debug>" PARENT_SCOPE)
            endif()
        else()
            set(debug_flags "${msvc_option_prefix}MDd")
            set(release_flags "${msvc_option_prefix}MD")
    ...
    elseif("${CMAKE_C_COMPILER_ID}" STREQUAL "GNU" OR "${CMAKE_C_COMPILER_ID}" STREQUAL "Clang" OR "${CMAKE_CXX_COMPILER_ID}" STREQUAL "GNU" OR "${CMAKE_CXX_COMPILER_ID}" STREQUAL "Clang")
        if(ARG_STATIC_RUNTIME)
            set(debug_flags "-static-libstdc++ -static-libgcc")
            set(release_flags "${debug_flags}")
        else()
            set(debug_flags "")
            set(release_flags "")
        endif()
    ...
    set(${ARG_RESULT_DEBUG_FLAGS} ${debug_flags} PARENT_SCOPE)
    set(${ARG_RESULT_RELEASE_FLAGS} ${release_flags} PARENT_SCOPE)
endfunction()
